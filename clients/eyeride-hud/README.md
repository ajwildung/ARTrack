...
- Pose/steer: send `{ "type": "steer", "kartId", "throttle": -1..1, "steer": -1..1 }` at ~20 Hz, **or** `{ "type": "pose", pose }` in `track_local` meters (`provider: "stub"` in M1; later `"arkit"` / `"vio"` / `"apriltag"`).
